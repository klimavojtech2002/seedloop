...
def _run_one(scenario: Scenario, seed: int) -> Sequence[object]:
    """Run ``scenario`` for one seed and return its recorded timeline.

    The CSPRNG shim is installed for the duration of the run and removed after, so ``os.urandom``
    and ``secrets`` draw from the seed without leaking the seeded source into later runs.
    """
    world = World(seed)
    with csprng_shim(substream(seed, "csprng")):
...
def check(
    ...
    *,
    ...
    ``"return"`` the sweep stops and returns the :class:`CheckResult`.
    """
